      ...
      setPaymentObject(
        data?.Payments?.map((payment) => {
          return {
            ...
            paymentStatus: payment.paymentStatus ? `CS_${payment.paymentStatus}` : "PTR_NA",
        ...
        <div style={isMobile ? {} : { marginLeft: "-16px" }}>
          <Card style={isMobile ? {marginBottom:"10px"} : {}}>
            <StatusTable>
              ...
              <LinkLabel style={isMobile ? {marginLeft:"0px"} : {}} onClick={() => printReciept("ptr", payment?.receiptNumber)}>{t("PTR_DOWNLOAD_RECEIPT")}</LinkLabel>
            </StatusTable>
          </Card>
        </div>
      )):<div>
